...
console.log('Starting API1 on port', port);  // Лог старта
...
app.get('/', (req, res) => {
  res.type('text/plain').send('Hello from API1 — всё ок!\n\nTry /items to see sample items.');
});
...
app.get('/items', async (req, res) => {
  try {
    ...
    const cachedItems = await redis.get('api1:items');
    if (cachedItems) {
      redis.quit();
      pgPool.end();
    ...
    const result = await pgPool.query('SELECT * FROM api1_items');
    const items = result.rows;

    await redis.set('api1:items', JSON.stringify(items), 'EX', 60);
...
app.listen(port, () => console.log(`API1 successfully started on ${port}`));